...
# Hoisted out of sensor.py: binary_sensor.py needs these too, and sensor.py was the wrong place
# for them to live once more than one platform needed them.
    ...
    cv.Schema(
        {
            ...
            # Datasheet 8.1.7: use the lowest setting that exceeds the highest active sensor
            # frequency. 10MHZ suits the ~2-8 MHz coils this driver is typically used with,
            # including the HomeWizard watermeter's ~5 MHz coils. A coil oscillating above 10 MHz
            # needs 33MHZ -- this default is a recommendation for the common case, not a value
            # that is safe for every coil.
            cv.Optional(CONF_DEGLITCH, default="10MHZ"): cv.enum(
